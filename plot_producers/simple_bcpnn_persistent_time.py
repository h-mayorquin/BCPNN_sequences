...
if run_tau_a:
    ...
    for index, tau_a_ in enumerate(tau_a_vector):
        nn = BCPNNPerfect(hypercolumns, minicolumns, g_w_ampa=g_w_ampa, g_w=g_w, g_a=g_a, tau_a=tau_a_, tau_m=tau_m,
                          sigma=sigma, G=G, tau_z_pre_ampa=tau_z_pre_ampa, tau_z_post_ampa=tau_z_post_ampa,
                          tau_p=tau_p, z_transfer=z_transfer, diagonal_zero=diagonal_zero,
    ...
    T_recall_theoretical_tau_a = simple_bcpnn_theo_recall_time(tau_a_vector, g_a, g_w_ampa, w_next, w_self)

    fig1 = plt.figure(figsize=(16, 12))
    ax1 = fig1.add_subplot(111)
    ax1.plot(tau_a_vector, T_recall_theoretical_tau_a, '-', lw=linewdith, label=r'Theory')
...
if run_g_a:
    ...
    for index, g_a_ in enumerate(g_a_vector):
        # Build the network
        nn = BCPNNPerfect(hypercolumns, minicolumns, g_w_ampa=g_w_ampa, g_w=g_w, g_a=g_a_, tau_a=tau_a, tau_m=tau_m,
                          sigma=sigma, G=G, tau_z_pre_ampa=tau_z_pre_ampa, tau_z_post_ampa=tau_z_post_ampa,
                          tau_p=tau_p, z_transfer=z_transfer, diagonal_zero=diagonal_zero,
                          strict_maximum=strict_maximum, perfect=perfect, k_perfect=k_perfect,
                          always_learning=always_learning)
...
if run_g_w:
    ...
    for index, g_w_ampa_ in enumerate(g_w_ampa_vector):
        ...
        # Build the network
        nn = BCPNNPerfect(hypercolumns, minicolumns, g_w_ampa=g_w_ampa_, g_w=g_w, g_a=g_a, tau_a=tau_a, tau_m=tau_m,
                          sigma=sigma, G=G, tau_z_pre_ampa=tau_z_pre_ampa, tau_z_post_ampa=tau_z_post_ampa,
                          tau_p=tau_p, z_transfer=z_transfer, diagonal_zero=diagonal_zero,
                          strict_maximum=strict_maximum, perfect=perfect, k_perfect=k_perfect,
                          always_learning=always_learning)
    ...
    T_recall_theoretical_g_w_ampa = simple_bcpnn_theo_recall_time(tau_a, g_a, g_w_ampa_vector, w_next, w_self)

    fig3 = plt.figure(figsize=(16, 12))

    ax3 = fig3.add_subplot(111)
    ax3.plot(g_w_ampa_vector, T_recall_theoretical_g_w_ampa, '-', lw=linewdith, label=r'Theory')
...
if run_w_next:
    ...
    T_recall_theoretical_w_next = simple_bcpnn_theo_recall_time(tau_a, g_a, g_w_ampa, w_next_vector, w_self)

    fig4 = plt.figure(figsize=(16, 12))

    ax4 = fig4.add_subplot(111)
    ax4.plot(w_next_vector, T_recall_theoretical_w_next, '-', lw=linewdith, label=r'Theory')
